...
:guilabel:`Product` lines by clickng :guilabel:`Add a product`.
...
To do that, go to
:menuselection:`Sales --> Create` to create a new quotation, and fill out the information on the
sales order form.
...
:guilabel:`green checkmark` to confirm the date. Finally, click :guilabel:`Confirm` to confirm the
sales order.
